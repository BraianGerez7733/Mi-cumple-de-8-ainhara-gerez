...
function initGravity() {
  ...
  function drawCard() {
    ...
    ctx.shadowColor = 'rgba(214,58,138,0.2)'
    ctx.shadowBlur = 18
    ...
    ctx.fill()
    ctx.shadowBlur = 0
    ctx.strokeStyle = 'rgba(255,111,183,0.55)'
    ctx.lineWidth = 2
    ctx.stroke()
    ...
    ctx.fillText('🎀 8 Añitos mágicos 🎀', 0, 16)
    ctx.font = `${Math.round(fs * 0.52)}px Nunito,sans-serif`
    ctx.fillStyle = '#b06cd0'
    ctx.fillText('¡Tocá todo para jugar! ✨', 0, 48)
    ctx.restore()
  }
  ...
  function moveDrag(p) {
    if (!dragging) return
    dragging._px = dragging.x
    dragging._py = dragging.y
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    if (!dragging) {
      ...
      canvas.style.cursor = hitTest(hoverPos.x, hoverPos.y) ? 'grab' : 'default'
    }
  })
  ...
  function loop(now) {
    ...
    particles.forEach(p => {
      if (p !== dragging) updateParticle(p, dtScale)
    })
  ...
  document.getElementById('toggle-gravity').addEventListener('click', () => {
    ...
    particles.forEach(p => {
      p.vy = (gravUp ? -3 : 2) * sf
      p.vx = (Math.random() - 0.5) * 4 * sf
    })
    card.vy = (gravUp ? -3 : 2) * sf
  })
  document.getElementById('rain-hearts').addEventListener('click', () => {
    const s = ['💖','💗','💕','💞','💓','🌸','✨','🦋','💝','💘']
    for (let i = 0; i < 10; i += 1) setTimeout(() => spawnEmoji(s[i % s.length]), i * 55)
  })
...
function initGame() {
  ...
  function particle(x, y, emoji) {
    for (let i = 0; i < 6; i += 1) {
      particles.push({ x, y, vx: (Math.random() - 0.5) * 5, vy: -Math.random() * 4, life: 1, emoji, size: 14 + Math.random() * 8 })
    }
  }
  ...
  function drawGround() {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      ctx.beginPath()
      ctx.arc(x, GROUND_Y + 25, 3, 0, Math.PI * 2)
      ctx.fill()
    }
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      ctx.beginPath()
      ctx.ellipse(cx, 50 + i * 25, 40, 20, 0, 0, Math.PI * 2)
      ctx.fill()
      ctx.beginPath()
      ctx.ellipse(cx + 20, 42 + i * 25, 28, 16, 0, 0, Math.PI * 2)
      ctx.fill()
    }
  }
  ...
  function drawCapy() {
    ...
    ctx.beginPath()
    ctx.ellipse(x + capy.w / 2, GROUND_Y + 18, 22, 7, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = '#c8a46e'
    ctx.beginPath()
    ctx.roundRect(x, y + 14, capy.w, capy.h - 14, 14)
    ctx.fill()
    ctx.fillStyle = '#c8a46e'
    ctx.beginPath()
    ctx.ellipse(x + capy.w - 16, y + 16, 18, 15, 0.1, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = '#b8936a'
    ctx.beginPath()
    ctx.ellipse(x + capy.w + 2, y + 20, 8, 6, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = '#3d1f00'
    ctx.beginPath()
    ctx.arc(x + capy.w - 8, y + 13, 3.5, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = 'white'
    ctx.beginPath()
    ctx.arc(x + capy.w - 7, y + 12, 1.2, 0, Math.PI * 2)
    ctx.fill()
    ...
    ctx.beginPath()
    ctx.arc(x + capy.w - 16, y + 2, 7, 0, Math.PI * 2)
    ctx.fill()
    const lo = capy.grounded ? Math.sin(scrollX * 0.15) * 4 : 0
    ctx.fillStyle = '#b8936a'
    ctx.beginPath()
    ctx.roundRect(x + 8, y + capy.h - 8, 10, 16 + lo, 4)
    ctx.fill()
    ctx.beginPath()
    ctx.roundRect(x + 22, y + capy.h - 8, 10, 12 - lo, 4)
    ctx.fill()
    ctx.beginPath()
    ctx.roundRect(x + capy.w - 22, y + capy.h - 8, 10, 12 + lo, 4)
    ctx.fill()
    ctx.beginPath()
    ctx.roundRect(x + capy.w - 10, y + capy.h - 8, 10, 16 - lo, 4)
    ctx.fill()
  }
  ...
  function drawObstacles() {
    obstacles.forEach(o => {
      if (o.type === 'puddle') {
        ...
        ctx.beginPath()
        ctx.ellipse(o.x + o.w / 2, o.y + o.h / 2, o.w / 2 - 5, o.h / 2 - 3, 0, 0, Math.PI * 2)
        ctx.fill()
      } else {
  ...
  function update() {
    ...
    for (let i = gifts.length - 1; i >= 0; i -= 1) {
      gifts[i].x -= speed * 0.8
      if (gifts[i].x < -40) {
        gifts.splice(i, 1)
        continue
...
async function loadRsvp() {
  ...
  if (!supabase) {
    list.innerHTML = '<div class="wall-loading">Sin conexión 🌐</div>'
    return
  }
  try {
    ...
    list.innerHTML = ''
    ;(data || []).forEach(r => {
      if (r.attendance === 'yes') yes += 1
      else if (r.attendance === 'no') no += 1
...
function initRsvp() {
  ...
  document.getElementById('send-rsvp-btn').addEventListener('click', async () => {
    ...
    fb.className = 'form-feedback'
    if (!name) {
      fb.textContent = '¡Escribí tu nombre! 💕'
      fb.classList.add('error')
      return
    }
    if (!attendance) {
      fb.textContent = '¡Elegí si vas o no! 🎈'
      fb.classList.add('error')
      return
    }
    ...
    try {
      ...
      fb.textContent = 'Error al guardar. Intenta de nuevo 😢'
      fb.classList.add('error')
      console.error(err)
    } finally {
      btn.disabled = false
...
async function loadRanking() {
  ...
  if (!supabase) {
    board.innerHTML = '<div class="wall-loading">Sin conexión 🌐</div>'
    return
  }
  try {
